...
export const findAlbums = async (data: searchParameters) => {
  ...
  if (!data.username) {
    const result = await albumsModel.find({ title: data.title }).populate("songs");
    return result;
  }
  const user = await User.findOne({ username: data.username });
  if (!data.title) {
    const result = await albumsModel.find({ owner: user?._id }).populate("songs");;
    return result;
  }
...
export const createAlbum = async (
  ...
  const newAlbum = await albumsModel.create({
    ...
    owner: user?._id,
    songs: validSongs,
    image: image || "https://i0.wp.com/sbcf.fr/wp-content/uploads/2018/03/sbcf-default-avatar.png?w=300&ssl=1",
  });
  return newAlbum;
};
